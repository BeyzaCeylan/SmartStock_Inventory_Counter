...
def detect_objects(image_path, confidence_threshold=0.25):
    ...
    for idx, detection in detections.iterrows():
        ...
        # Sadece ürün olabilecek nesneleri say (person, animal gibi sınıfları hariç tut)
        class_name = detection['name']
        product_classes = ['bottle', 'cup', 'bowl', 'book', 'vase', 'teddy bear', 
                          'cell phone', 'remote', 'keyboard', 'laptop', 'mouse', 
                          'tv', 'microwave', 'oven', 'toaster', 'refrigerator', 
                          'clock', 'chair', 'couch', 'bed', 'dining table', 'toilet']
        
        # Eğer nesne bir ürün olabilecek sınıftaysa say
    ...
    # Toplam ürün sayısını resmin üstüne yaz
    cv2.putText(img, f"Toplam Urun Sayisi: {object_count}", (10, 30), 
                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
    
    print(f"Tespit edilen toplam ürün sayısı: {object_count}")
